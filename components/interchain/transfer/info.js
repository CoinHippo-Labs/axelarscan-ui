...
export default ({ data, buttons }) => {
  ...
  const steps = toArray([
    ...
    !['send_token', 'wrap', 'erc20_transfer'].includes(type) && {
      id: 'confirm',
      title: confirm ? 'Confirmed' : send && source_chain_data?.chain_type === 'evm' ? 'Waiting for Finality' : 'Confirm',
    ...
    getChainData(source_chain_data?.id, chains_data)?.chain_type === 'evm' && {
      id: 'vote',
      title: vote ? 'Approved' : confirm ? 'Approving' : 'Approve',
    ...
    getChainData(destination_chain_data?.id, chains_data)?.chain_type === 'evm' && {
      id: 'command',
      title: command?.executed || command?.transactionHash ? 'Received' : 'Execute',
