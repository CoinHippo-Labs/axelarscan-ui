...
export default ({ n }) => {
  ...
  useEffect(() => {
    ...
    triggering()
    const interval = setInterval(() => triggering(true), (address || ['/gmp/search'].includes(pathname) ? 3 : 0.2) * 60 * 1000)
    return () => {
          ...
          columns={[
            {
              ...
              Cell: props => {
                ...
                const { chain } = { ...call }
                const source_chain_data = getChain(chain, evm_chains_data)
            ...
            },
            {
              ...
              Cell: props => {
                ...
                const chain_data = getChain(props.value, evm_chains_data)
                const { name, image, explorer } = { ...chain_data }
                          ...
                          >
                            <EnsProfile
                              ...
                              fallback={(
                                <div className="h-6 flex items-center text-blue-600 dark:text-white font-bold">
                                  <span className="xl:hidden">
                                    {ellipse(destinationContractAddress, 6)}
                                  </span>
                                  <span className="hidden xl:block">
                                    {ellipse(destinationContractAddress, 8)}
                                  </span>
                                </div>
                              )}
                            />
            ...
            },
            {
              ...
              Cell: props => {
                ...
                const source_chain_data = getChain(chain, evm_chains_data)
                const destination_chain_data = getChain(destinationChain, evm_chains_data)
                const steps = [{
                  title: 'Contract Call',
