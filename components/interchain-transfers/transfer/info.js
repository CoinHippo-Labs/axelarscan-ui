...
export default ({ data }) => {
  ...
  const steps = toArray([
    ...
    getChainData(destination_chain_data?.id, chains_data)?.chain_type === 'evm' && {
      id: 'command',
      title: command?.executed || command?.transactionHash ? 'Received' : 'Receive',
    ...
    getChainData(destination_chain_data?.id, chains_data)?.chain_type === 'cosmos' && destination_chain_data.id !== 'axelarnet' && {
      id: 'ibc_send',
      title: ibc_send?.ack_txhash || (ibc_send?.recv_txhash && !ibc_send.failed_txhash) ? 'Received' : 'Receive',
    ...
    destination_chain_data?.id === 'axelarnet' && {
      id: 'axelar_transfer',
      title: axelar_transfer ? 'Received' : 'Receive',
