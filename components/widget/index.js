...
const Widget = ({ title = null, description = null, right = null, className = '', contentClassName = '', titleSubTitleClassName = '', onClick, children }) => {
  return (
    <div onClick={onClick} className={`widget w-full ${className.includes('bg-') ? '' : 'bg-white dark:bg-gray-900'} ${className.includes('border-') ? '' : 'border border-gray-100 dark:border-gray-800'} rounded-2xl ${className.includes('p-') ? '' : 'p-4'} ${className}`}>
