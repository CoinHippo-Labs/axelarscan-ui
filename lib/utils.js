...
export const ellipse = (string, length = 10, prefix = '') => !string ? '' : string.length < (length * 2) + 3 ? string : `${prefix}${string.replace(prefix, '').slice(0, length)}...${string.replace(prefix, '').slice(-length)}`
