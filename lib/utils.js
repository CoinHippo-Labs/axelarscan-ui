...
export const number_format = (number, format, is_exact) => {
  ...
  if (string?.toLowerCase().endsWith('t') && string.split(',').length > 0) {
...
export const capitalize = s => typeof s !== 'string' ? '' : s.trim().split(' ').join('_').split('-').join('_').split('_').map(x => x.trim()).filter(x => x).map(x => `${x.substr(0, 1).toUpperCase()}${x.substr(1)}`).join(' ')
...
export const _total_time_string = (total_time, a, b) => {
  if (typeof total_time === 'number') {
    total_time = parseInt(total_time)
  }
  return total_time < 60 ?
  `${total_time || 0}s` : total_time < 60 * 60 ?
  `${Math.floor(total_time / 60)} min${total_time % 60 > 0 ? ` ${total_time % 60}s` : ''}` : total_time < 24 * 60 * 60 ?
  moment.utc(total_time * 1000).format('HH:mm:ss') : a & b ? `${moment(b).diff(moment(a), 'days')} day` : `${moment().diff(moment().subtract(total_time, 'seconds'), 'days')} day`
}
export const total_time_string = (a, b) => {
  ...
  return _total_time_string(total_time, a, b)
}
...
export const chart_color = (theme, type) => {
  return ['day'].includes(type) ?
    theme === 'dark' ? '#f88e56' : '#f56417' :
