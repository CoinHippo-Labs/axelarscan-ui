    ...
    while (pageKey) {
      ...
      data = data
        .map(v => {
          ...
          const i = info?.find(i =>
            equals_ignore_case(i?.address, consensus_address)
          )
  ...
  for (const d of no_broadcaster_data) {
    ...
    if (
      ...
      broadcaster_address = broadcasters_data
        .find(_d =>
          equals_ignore_case(_d?.operator_address, operator_address)
        )?.broadcaster_address
...
export const all_validators_status = async (
  ...
  data = data
    .map(v => {
      ...
      return {
        ...v,
        stale_heartbeats: (response?.data || [])
          .findIndex(d =>
            equals_ignore_case(d?.sender, broadcaster_address)
          ) < 0,
      }
    })
  ...
  if (total > max_size && is_unlimit) {
    txs = txs
      .filter(tx =>
        ...
        first_load_txs
          .findIndex(_tx =>
            equals_ignore_case(_tx?.txhash, tx?.txhash)
          ) < 0
      )
  }
