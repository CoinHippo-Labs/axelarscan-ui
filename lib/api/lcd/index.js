...
import { proxy_address } from '../proxy-address'
...
export const all_validators_broadcaster = async (
  ...
  const no_broadcaster_data = data
    .filter(v =>
  ...
  if (no_broadcaster_data.length > 1) {
    ...
    broadcasters_data = (response?.data || [])
      .map(d => {
        const {
          id,
          stdout,
...
export const all_validators_status = async (
  ...
  let response = await getHeartbeats(
    {
      size: 500,
    },
  )
...
export const chain_maintainer = async (
  ...
  for (const chain of chains) {
    const response = await chain_maintainers(
      {
    ...
    data[chain] = Array.isArray(maintainers) ?
      maintainers :
      []
  }
