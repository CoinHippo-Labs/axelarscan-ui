...
import { equals_ignore_case, to_json, sleep } from '../../utils'
...
export const getBlock = async (
  ...
  if (header) {
    ...
    proposer_address = base64ToBech32(
      proposer_address,
      process.env.NEXT_PUBLIC_PREFIX_CONSENSUS,
    )
    num_txs = typeof txs?.length === 'number' ?
      txs.length :
      -1
  }
  ...
  if (validators) {
    for (let i = 0; i < validators.length; i++) {
      ...
      if (validator) {
        ...
        if (key) {
          consensus_address = pubKeyToBech32(
            key,
            process.env.NEXT_PUBLIC_PREFIX_CONSENSUS,
          )
        }
        ...
        if (
          ...
          validator = await validator_self_delegation(
            validator,
            validators_data,
            true,
          )
...
export const all_validators = async (
  ...
  while (pageKey) {
    const response = await _validators(
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
          pageKey :
          undefined,
      },
      validators_data,
  ...
  if (
    ...
    const response = await uptimes(
      {
        query: {
          range: { height: { gt: latest_block_height - 1 - num_uptime_blocks } }
        },
        aggs: {
          uptimes: {
    ...
    if (response?.data) {
      data = data.map(v => {
        ...
        return {
          ...v,
          uptime: (response.data[consensus_address] || 0) *
            100 /
            (total || num_uptime_blocks),
        }
      }).map(v => {
        const {
          uptime,
  ...
  if (
    ...
    while (pageKey) {
      ...
      data = data.map(v => {
        ...
        const i = info?.find(i => equals_ignore_case(i?.address, consensus_address))
        ...
        if (i) {
          ...
          start_proxy_height = start_proxy_height ||
            start_height
          ...
          return {
            ...
            jailed_until: jailed_until &&
              moment(jailed_until).valueOf(),
            tombstoned: typeof tombstoned === 'boolean' ?
              tombstoned :
              undefined,
            missed_blocks_counter,
          }
...
export const all_validators_broadcaster = async (
  ...
  if (transactions_data) {
    data = data.map(v => {
      ...
      const transaction_data = transactions_data.find(t =>
        !t?.code &&
      ...
      return {
        ...v,
        start_proxy_height,
        broadcaster_address: messages?.find(m =>
          m?.['@type']?.includes('RegisterProxy') &&
          equals_ignore_case(m.sender, operator_address)
        )?.proxy_addr,
  ...
  const no_broadcaster_data = data.filter(v =>
  ...
  if (no_broadcaster_data.length > 1) {
    ...
    broadcasters_data = response?.data?.map(d => {
      const {
        id,
        stdout,
      } = { ...d }
      ...
      return {
        operator_address: _.last(id?.split(' ')),
  ...
  for (const d of no_broadcaster_data) {
    ...
    const index = data.findIndex(v => equals_ignore_case(v?.operator_address, operator_address))

    if (index > -1 && !broadcaster_address) {
      broadcaster_address = broadcasters_data?.find(_d => equals_ignore_case(_d?.operator_address, operator_address))?.broadcaster_address

      if (!broadcaster_address) {
        response = await axelard(
          {
            cmd: `axelard q snapshot proxy ${operator_address}`,
            cache: true,
            cache_timeout: 5,
          },
        )

        broadcaster_address = to_json(response?.stdout)?.address
      }
...
export const all_validators_status = async (
  ...
  if (to_json(stdout)) {
    ...
    data = _.orderBy(
      data.map(v => {
        return {
          ...v,
          deregistering: (operator_addresses || []).includes(v.operator_address),
        }
      }),
      [
        'deregistering',
  ...
  data = data.map(v => {
    ...
    return {
      ...v,
      stale_heartbeats: response?.data?.findIndex(d => equals_ignore_case(d?.sender, broadcaster_address)) < 0,
    }
  })
...
export const chain_maintainer = async (
  ...
  for (const chain of chains) {
    const response = await axelard(
      {
        cmd: `axelard q nexus chain-maintainers ${chainManager.maintainer_id(chain, chains_data)} -oj`,
        cache: true,
        cache_timeout: 1,
      },
    )
    ...
    if (maintainers) {
      data[chain] = Object.values(maintainers)
    }
  }
...
export const validator_self_delegation = async (
  ...
  if (
    ...
    self_delegation = validators_data?.find(v =>
      typeof v.self_delegation === 'number' &&
      equals_ignore_case(v.operator_address, operator_address)
    )?.self_delegation
    ...
    if (!self_delegation &&
      ...
      const response = await staking_delegations_address(
        operator_address,
        delegator_address,
      )
      ...
      self_delegation = Number(shares || '0')
    }
...
export const all_proposal_votes = async (
  ...
  while (pageKey) {
    const response = await _proposal_votes(
      id,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
...
export const transactions = async (
  ...
  if (tx_responses) {
    ...
    tx_responses = tx_responses.map(d => {
      let {
        height,
      } = {  ...d }
      ...
      return {
        ...
        fee: transactionManager.fee(
          d,
          assets_data,
        ),
        symbol: transactionManager.symbol(
          d,
          assets_data,
        ),
        ...
        activities: transactionManager.activities(
          d,
          assets_data,
        ),
      }
...
export const transactions_by_events = async (
  ...
  while (
    ...
    const _pageKey = (is_unlimit || total <= max_size) && pageKey && typeof pageKey === 'string' ?
      pageKey :
      undefined
    const _offset = total +
      (total % page_size === 0 ?
  ...
  if (total > max_size && is_unlimit) {
    txs = txs
      .filter(tx =>
        !first_load_txs ||
        first_load_txs.findIndex(_tx => equals_ignore_case(_tx?.txhash, tx?.txhash)) < 0
      )
  }
...
export const all_bank_balances = async (
  ...
  while (pageKey) {
    const response = await _bank_balances(
      address,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
...
export const all_staking_delegations = async (
  ...
  while (pageKey) {
    const response = await _staking_delegations(
      address,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
...
export const all_staking_redelegations = async (
  ...
  while (pageKey) {
    const response = await _staking_redelegations(
      address,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
...
export const all_staking_unbonding = async (
  ...
  while (pageKey) {
    const response = await _staking_unbonding(
      address,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
...
export const all_delegations = async (
  ...
  while (pageKey) {
    const response = await _delegations(
      address,
      {
        ...params,
        'pagination.key': pageKey && typeof pageKey === 'string' ?
