...
export const all_validators = async (params, validators_data, status, addresses, latest_block, denoms) => {
  ...
  if (latest_block && (['active', 'inactive', 'deregistering'].includes(status) || addresses?.length > 0)) {
    ...
    const response = await uptimes({
      query: {
        range: { height: { gt: latest_block - 1 - num_uptime_blocks } }
      },
      aggs: {
        uptimes: {
  ...
  if (status || addresses?.length > 0 || params) {
    pageKey = true
    while (pageKey) {
...
export const transactions = async (params, denoms) => {
  ...
  if (response?.tx_responses) {
    response.tx_responses = response.tx_responses.map(r => {
      const activities = transactionManager.activities(r, denoms)
      return {
        ...r,
        height: Number(r.height),
        status: transactionManager.status(r),
        type: transactionManager.type(r),
        sender: transactionManager.sender(r),
        fee: transactionManager.fee(r, denoms),
        symbol: transactionManager.symbol(r, denoms),
        gas_used: transactionManager.gas_used(r),
        gas_limit: transactionManager.gas_limit(r),
        memo: transactionManager.memo(r),
        activities,
      }
    })
    response = {
