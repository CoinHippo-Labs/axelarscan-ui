...
export const all_validators = async (params, validators_data, status, addresses, latest_block, denoms) => {
  ...
  if (status || addresses?.length > 0) {
    ...
    while (pageKey) {
      ...
      data = data.map(v => {
        const info = infos?.find(i => equals_ignore_case(i?.address, v?.consensus_address))
        if (info) {
          return {
            ...v,
...
export const all_validators_broadcaster = async (validators_data, addresses, denoms) => {
  ...
  let data = validators_data || [],
    response = await transactions_by_events(`message.action='RegisterProxy'`, null, null, true, denoms)
  if (response?.data) {
    data = data.map(v => {
...
export const transactions_by_events = async (events, data, validator_address, is_unlimit, denoms) => {
