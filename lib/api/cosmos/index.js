  ...
  if (
    ...
    if (response?.data) {
      data = data.map(v => {
        ...
        return {
          ...v,
          uptime: (response.data[consensus_address] || 0) * 100 / (total || num_uptime_blocks),
        }
      }).map(v => {
        const {
          uptime,
...
export const all_validators_status = async validators_data => {
  ...
  if (to_json(stdout)) {
    const deregistering_addresses = to_json(stdout)?.operator_addresses || []
    ...
    data = _.orderBy(
      ...
      ['deregistering', 'description.moniker'],
      ['asc', 'asc'],
    )
  }
...
export const chain_maintainer = async (id, chains_data) => {
  ...
  for (let i = 0; i < chains.length; i++) {
    ...
    const response = await axelard({
      cmd: `axelard q nexus chain-maintainers ${chainManager.maintainer_id(id, chains_data)} -oj`,
      cache: true,
      cache_timeout: 5,
    })
    ...
    data = _.orderBy(_.uniqBy(_.concat(data, response?.proposals?.map(p => {
      ...
      return {
        ...
        total_deposit: total_deposit?.map(d => {
          return {
            ...d,
            amount: assetManager.amount(d?.amount, d?.denom, denoms),
            symbol: assetManager.symbol(d?.denom, denoms),
          }
        }),
        final_tally_result: Object.fromEntries(
          Object.entries({ ...final_tally_result }).map(([k, v]) =>
            [k, assetManager.amount(Number(v), denoms?.[0]?.denom, denoms)]
          )
        ),
      }
...
export const transactions_by_events = async (events, data, is_unlimit, denoms, max_size = 500) => {
  const page_size = 50
...
export const validator_sets = async (height, params) => {
  const path = `/validatorsets/${height || 'latest'}`
  params = { ...params, cache_timeout: 1 }
  return await request(path, params)
}
