...
import { rand_image, to_json } from '../../utils'
        ...
        if (_validator.description) {
          if (_validator.description.identity) {
            if (validators_data?.findIndex(v => v?.description?.image && v.operator_address === _validator.operator_address) > -1) {
              ...
              _validator.description.image = _validator.description.image || (_validator.description.moniker?.toLowerCase().startsWith('axelar-core-') ? '/logos/chains/axelar.png' : rand_image(i))
            }
          }
          else {
            _validator.description.image = _validator.description.image || (_validator.description.moniker?.toLowerCase().startsWith('axelar-core-') ? '/logos/chains/axelar.png' : rand_image(i))
...
export const allValidatorsStatus = async validators_data => {
  ...
  if (to_json(response?.stdout)) {
    const illegible_addresses = to_json(response.stdout)?.validators?.filter(v => Object.values(v?.tss_illegibility_info || {}).findIndex(value => value) > -1)
    data = _.orderBy(data.map(v => {
      return {
        ...v,
    ...
    data = _.orderBy(_.uniqBy(_.concat(data, response.proposals?.map(_proposal => {
      return {
        ...
        total_deposit: _proposal?.total_deposit?.map(deposit => {
          return {
            ...deposit,
            amount: denom_manager.amount(deposit?.amount, deposit?.denom, denoms),
            symbol: denom_manager.symbol(deposit?.denom, denoms),
          }
        }),
        final_tally_result: Object.fromEntries(Object.entries(_proposal?.final_tally_result || {}).map(([key, value]) => [key, denom_manager.amount(Number(value), denoms?.[0]?.denom, denoms)]))
      }
