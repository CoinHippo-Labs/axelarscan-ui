...
export const status = async (params, _status) => {
  ...
  if (response && Number(response?.earliest_block_height) > 0) {
    const block_variance = 1, num_avg_block_time_blocks = Number(process.env.NEXT_PUBLIC_NUM_AVG_BLOCK_TIME_BLOCKS) || 100
    ...
    const earliest_block_height_for_cal = Number(response.latest_block_height) - num_avg_block_time_blocks
