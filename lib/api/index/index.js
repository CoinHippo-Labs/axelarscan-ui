  ...
  if (response?.data) {
    response.data = response.data.map(d => {
      return {
        ...
        fee: transactionManager.fee(d, denoms),
        symbol: transactionManager.symbol(d, denoms),
        gas_used: transactionManager.gas_used(d),
        gas_limit: transactionManager.gas_limit(d),
        memo: transactionManager.memo(d),
        activities: transactionManager.activities(d, denoms),
      }
    })
  }
...
export const keygens = async (params, is_success = true) => {
  ...
  let response = await request(path, params)
  if (response?.aggs?.keygens?.buckets) {
    response = {
      data: Object.fromEntries(response.aggs.keygens.buckets.map(r => [r.key, r.doc_count])),
      total: response.total,
    }
  }
  return response
...
export const sign_attempts = async params => {
  ...
  let response = await request(path, params)
  if (response?.aggs?.signs?.buckets) {
    response = {
      data: Object.fromEntries(response.aggs.signs.buckets.map(r => [r.key, r.doc_count])),
      total: response.total,
    }
  }
  return response
...
export const transfers = async params => {
  ...
  else if (response?.aggs?.stats?.buckets) {
    response = {
      data: _.orderBy(response.aggs.stats.buckets.map(c => {
        return {
          timestamp: c.key,
          volume: c.volume?.value || 0,
          num_txs: c.doc_count,
...
export const token_sents = async params => {
  const path = '/token_sent_events/_search'
  params = {
    size: 0,
    ...params,
