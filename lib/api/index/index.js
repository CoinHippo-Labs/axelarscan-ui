    ...
    response = {
      data: Object.fromEntries(
        buckets.map(r => {
          ...
          return [
            key,
            period_height?.buckets && period_height.buckets.length <= 100000 ?
      ...
      data: buckets.map(c => {
        return {
          ...
          volume: c.volume?.value ||
            0,
          cumulative_volume: c.cumulative_volume?.value ||
            0,
          num_txs: c.doc_count,
        }
      }),
...
export const token_sents = async params => {
  let response = await request(
    '/token_sent_events/_search',
    {
      size: 0,
      ...params,
...
export const fieldsToObj = (
  ...
  const obj = {},
    fields = Object.keys(fields_data)
      .filter(f => f && !f.endsWith('.keyword'))
  ...
  fields
    ...
    .forEach(f => {
      ...
      obj[f] = array_fields.includes(f) ?
        value :
        _.head(value)
    })

  const nested_fields = fields.filter(f => f.includes('.'))
  const csv = [
    nested_fields.join(','),
  ]
