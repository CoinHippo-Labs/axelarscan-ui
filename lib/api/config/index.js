...
export const chains = async params => {
  ...
  return {
    ...
    cosmos: response?.cosmos?.filter(a => !a?.is_staging || is_staging).map(a => {
      return a?.id === 'axelarnet' ? { ...a, explorer: { ...a?.explorer, url: a?.explorer?.url?.replace('https://axelarscan.io', process.env.NEXT_PUBLIC_SITE_URL).replace('https://testnet.axelarscan.io', process.env.NEXT_PUBLIC_SITE_URL) } } : a
    }) || [],
  }
}
