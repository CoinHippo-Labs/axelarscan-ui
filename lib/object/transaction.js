...
export const transactionManager = {
  ...
  sender: data => {
    ...
    return _.head(
      [
        equals_ignore_case(transactionManager.type(data), 'MsgDelegate') && 'delegator_address',
        'sender',
        'signer',
      ]
      .filter(f => f)
  ...
  recipient: data => {
    ...
    return _.head(
      [
        equals_ignore_case(transactionManager.type(data), 'MsgDelegate') &&
          'validator_address',
        equals_ignore_case(transactionManager.type(data), 'MsgUndelegate') &&
          'delegator_address',
        'recipient',
      ]
      .filter(f => f)
    ...
    if (
      ...
      activities = messages.flatMap(m => {
        ...
        timeout_timestamp = timeout_timestamp &&
          (Number(timeout_timestamp) / 1000000)
      ...
      activities = messages.flatMap(m => {
        ...
        tx_id = _.head(events)?.tx_id ||
          tx_id

        status = _.head(events)?.status ||
          status

        events = events?.flatMap(e =>
          Object.entries(e)
            .filter(([k, v]) => typeof v === 'object' && v)
            .map(([k, v]) => {
              return {
                event: k,
                ...v,
    ...
    if (!(activities?.length > 0)) {
      activities = logs?.flatMap(l => {
        ...
        events = events?.flatMap(e => {
          ...
          if (equals_ignore_case(type, 'delegate')) {
            const _events = [],
              template = {
                type,
                log,
          ...
          else if (equals_ignore_case(type, 'unbond')) {
            const _events = [],
              template = {
                type,
                log,
          ...
          else if (equals_ignore_case(type, 'transfer')) {
            const _events = [],
              template = {
                type,
                log,
        ...
        return events?.findIndex(e => delegate_event_types.includes(e?.type)) > -1 ?
          events.filter(e => delegate_event_types.includes(e?.type)) :
          events?.findIndex(e => transfer_event_types.includes(e?.type)) > -1 ?
            events.filter(e => transfer_event_types.includes(e?.type)) :
            _.assign.apply(
              _,
              events,
            )
