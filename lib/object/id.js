export const type = id => {
  ...
  return !id ? null : id.match(txRegEx) ? 'gmp' : !isNaN(id) ? 'block' : id.match(validatorRegEx) ? 'validator' : id.match(accountRegEx) || Object.values(cosmosChainsAddressRegEx).findIndex(r => id.match(r)) > -1 ? 'account' : 'tx'
}