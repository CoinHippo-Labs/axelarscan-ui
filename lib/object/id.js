export const type = id => {
  ...
  return !id ? null : !isNaN(id) ? 'block' : id.match(validatorRegEx) ? 'validator' : id.match(accountRegEx) || Object.values(cosmosChainsAddressRegEx).findIndex(r => id.match(r)) > -1 ? 'account' : id.match(txRegEx) ? 'gmp' : 'tx'
}