...
export const getChain = (id, chains) => {
  const chain_data = chains?.find(c =>
    [c?.id?.toLowerCase(), c?.maintainer_id?.toLowerCase()]
      .concat(Object.keys({ ...c?.overrides }))
      .filter(i => i).includes(id?.toLowerCase()) ||
    c?.prefix_chain_ids?.findIndex(p => id?.startsWith(p)) > -1
  )
  return chain_data && {
    ...chain_data,
